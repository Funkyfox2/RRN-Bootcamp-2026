...
/**
 * Simple single-jointed arm driven by a NEO through a 12:1 gearbox, controlled with a trapezoidal
 * motion profile and a proportional-only closed-loop controller.
 *
 * <p>This is the minimal YAMS arm example: no absolute encoder, no gravity feedforward, duty-cycle
 * override available alongside position commands. The arm angle is tracked with the NEO's built-in
 * relative encoder; the encoder position is considered valid from power-on (no homing routine
 * required for this demo).
 *
 * <p>Soft limits constrain commanded motion to -30..100 deg. Hard limits (-100..200 deg) are the
 * absolute physical stops modeled in simulation.
 */
public class ArmSubsystem extends SubsystemBase {
  // NEO on CAN ID 1.  Change this to match your robot's CAN bus assignment.
  private final SparkMax armMotor = new SparkMax(2, SparkLowLevel.MotorType.kBrushless);
  ...
  /**
   * Drive the arm open-loop at the given duty cycle. Useful for manual adjustment and as a safe
   * default command.
   *
  ...
  /**
   * Move the arm to a fixed angle using the closed-loop controller. The trapezoidal profile ramps
   * velocity so the arm does not slam into the setpoint.
   *
