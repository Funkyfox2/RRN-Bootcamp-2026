...
/**
 * Simple flywheel shooter subsystem demonstrating continuous closed-loop velocity control with
 * SimpleMotorFeedforward.
 *
 * <p>Unlike a positional mechanism (arm, elevator), a flywheel's setpoint is angular velocity, and
 * the closed-loop controller runs indefinitely to hold that speed against disturbances such as ball
 * loading drag. A SimpleMotorFeedforward does most of the work; the PID term trims steady-state
 * error that the feedforward cannot model (e.g., manufacturing variance, belt stretch, or load
 * changes).
 *
 * <p>Mechanism summary:
 *
 * <ul>
 *   <li>Motor: REV NEO on a SparkMAX, CAN ID 1
 *   <li>Gearing: 12:1 reduction (3-stage, 4:1 each)
 *   <li>Wheel: 4-inch diameter, ~1 lb effective mass
 *   <li>Top speed at 12 V: roughly 450 RPM at the wheel after reduction
 * </ul>
 *
 * <p>Characterization constants (obtained via SysId on this exact mechanism):
 *
 * <ul>
 *   <li>kS = 0.27937 V -- static friction; volts needed before the shaft starts moving
 *   <li>kV = 0.089836 V/RPS -- back-EMF slope; higher kV means more voltage per unit speed
 *   <li>kA = 0.014557 V/(RPS/s) -- inertia term; small because the wheel is light
 * </ul>
 */
public class EndeffectorSubsytem extends SubsystemBase {
  ...
  public double getEndEffectorLinearPosition() {
    // 12 gear reduction
    // 2 " fly wheal
    return (encoder.getPosition()) * 3.14 * 2;
  }
  ...
  /**
   * Drives the flywheel in open-loop at a fixed duty cycle. Useful for manual tuning or fallback if
   * characterization data is unavailable.
   *
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber("EndeffectorSpeed", shooter.getLinearVelocity().in(InchesPerSecond));
  }
